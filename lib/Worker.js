...
function getScore(room, now) {
  return Q.Promise(function (resolve, reject, notify) {
    return getTopNode(room.nodeId).then(function (node) {
      // Tack-on the comment count
      room.commentCount = node.commentCount;
      // Calculate the score
...
function updateRoom(location, room) {
  return Q.Promise(function (resolve, reject, notify) {
    ...
    $ref.update({'score': room.score, 'commentCount': room.commentCount}, resolve);
  });
}
  ...
  return Q.Promise(function(resolve, reject, notify) {
    ...
    $ref.once('value', function ($rooms) {
      ...
      return CurrentTime($firebase)
        ...
        console.log('Finished calculating Room scores (test)');
  ...
  return Q.promise(function (resolve, reject, notify) {
    ...
    $ref.once('value', function ($snap) {
      $snap.forEach(function ($item) {
        ...
        // A rooms is dead if its score is above 0... E.g. a good score is in the negative (this is for sorting purposes)
